 ...
 *------------------------------------------*/
static bool mapif_parse_AutofarmReward(int32 fd) {
	const uint32 account_id = RFIFOL(fd, 4);
	const uint32 char_id = RFIFOL(fd, 8);
	const bool stackable = RFIFOB(fd, 12) != 0;
	struct item incoming = {};
	memcpy(&incoming, RFIFOP(fd, 13), sizeof(struct item));
	if (account_id == 0 || char_id == 0 || incoming.nameid == 0 || incoming.amount <= 0)
		return false;

	struct s_storage stor = {};
	stor.stor_id = 0;
	if (!storage_fromsql(account_id, &stor))
		return false;

	int32 remaining = incoming.amount;
	while (remaining > 0) {
		int32 slot = -1;
		if (stackable) {
			for (int32 i = 0; i < MAX_STORAGE; ++i) {
				struct item existing = stor.u.items_storage[i];
				if (existing.nameid == 0 || existing.amount <= 0 || existing.unique_id != 0)
					continue;
				struct item a = existing;
				struct item b = incoming;
				a.id = b.id = 0;
				a.amount = b.amount = 0;
				if (memcmp(&a, &b, sizeof(struct item)) == 0 && existing.amount < MAX_AMOUNT) {
					slot = i;
					break;
				}
			}
		...
		const int32 capacity = stackable && stor.u.items_storage[slot].nameid != 0
			? MAX_AMOUNT - stor.u.items_storage[slot].amount : MAX_AMOUNT;
		const int32 chunk = std::min(remaining, std::max(1, capacity));
		if (stor.u.items_storage[slot].nameid == 0) {
			stor.u.items_storage[slot] = incoming;
			stor.u.items_storage[slot].amount = static_cast<int16>(chunk);
			stor.u.items_storage[slot].id = 0;
			stor.amount++;
		} else {
			stor.u.items_storage[slot].amount = static_cast<int16>(stor.u.items_storage[slot].amount + chunk);
		}
		remaining -= chunk;
		if (!stackable)
			break;
		...
		stor.dirty = true;
	// storage_tosql() returns true on success. Returning its value prevents
	// the map-server from leaving the item on the floor after a valid save.
	return storage_tosql(account_id, &stor);
...
bool inter_storage_parse_frommap(int32 fd)
{
	switch(RFIFOW(fd,0)){
		...
		case 0x30c4: mapif_parse_AutofarmReward(fd); break;
