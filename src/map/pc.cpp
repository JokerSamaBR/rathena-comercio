	...
	for( i = 0; i < MAX_INVENTORY; i++ ) { // Check for Rentals on Inventory
		...
		if( sd->inventory.u.items_inventory[i].expire_time <= time(nullptr) ) {
			if (sd->inventory_data[i]->unequip_script) {
				auto it = util::vector_get(AA_ITEMIDS, sd->inventory.u.items_inventory[i].nameid);
...
bool pc_authok(map_session_data *sd, uint32 login_id2, time_t expiration_time, int32 group_id, struct mmo_charstatus *st, bool changing_mapservers)
{
	...
	if (pc_is_maxjoblv(sd) && sd->status.job_exp > MAX_LEVEL_JOB_EXP) {
		...
		clif_updatestatus(*sd, SP_JOBEXP);
				...
				// Load AutoAttack configuration from SQL after character data is available.
					aa_load(sd);
...
int32 pc_useitem(map_session_data *sd,int32 n)
{
	...
	if( sd->st != nullptr ){
		// Interactive item scripts (menu/select/next/input) remain attached
		// until the player sends the response. Free only completed scripts.
		if( sd->st->state == END ){
...
static void pc_calcexp(map_session_data *sd, t_exp *base_exp, t_exp *job_exp, block_list *src)
{
	...
	if (sd->sc.getSCE(SC_PERIOD_PLUSEXP_2ND))	// Increase Jexp as well
		...
		const int32 season_base_rate = customrate_base_rate(sd);
		if (*base_exp && season_base_rate > 1)
			*base_exp = *base_exp > (MAX_EXP / season_base_rate) ? MAX_EXP : *base_exp * season_base_rate;

		const int32 season_job_rate = customrate_job_rate(sd);
		if (*job_exp && season_job_rate > 1)
			*job_exp = *job_exp > (MAX_EXP / season_job_rate) ? MAX_EXP : *job_exp * season_job_rate;
