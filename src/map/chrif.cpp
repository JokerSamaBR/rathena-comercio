...
bool chrif_autofarm_reward(uint32 account_id, uint32 char_id, const struct item* reward, bool stackable) {
	if (reward == nullptr || char_fd < 0 || !chrif_connected)
		return false;

	constexpr uint16 opcode = 0x30c4;
	constexpr uint16 packet_size = static_cast<uint16>(2 + 2 + 4 + 4 + 1 + sizeof(struct item));
	WFIFOHEAD(char_fd, packet_size);
	WFIFOW(char_fd, 0) = opcode;
	WFIFOW(char_fd, 2) = packet_size;
	WFIFOL(char_fd, 4) = account_id;
	WFIFOL(char_fd, 8) = char_id;
	WFIFOB(char_fd, 12) = stackable ? 1 : 0;
	memcpy(WFIFOP(char_fd, 13), reward, sizeof(struct item));
	WFIFOSET(char_fd, packet_size);
	return true;
}
