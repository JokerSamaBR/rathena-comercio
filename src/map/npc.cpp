...
size_t npc_script_event( map_session_data& sd, enum npce_event type ){
	...
	for( struct script_event_s& evt : vector ){
		// DailyHuntMaster only needs the kill event while a matching hunt is active.
		// This check must happen before npc_event_sub(), because the latter enqueues
		// the event before the script's own `if (...) end;` guards can run.
		if( type == NPCE_KILLNPC && evt.event != nullptr && evt.event->nd != nullptr &&
			strcmpi( evt.event->nd->exname, "DailyHuntMaster" ) == 0 ){
			if( pc_readreg2( &sd, "#HuntActive" ) == 0 || pc_readreg2( &sd, "#HuntTarget" ) != sd.killedrid )
				continue;
		}
