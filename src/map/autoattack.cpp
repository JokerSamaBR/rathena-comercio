...
std::vector<t_itemid> AA_ITEMIDS = { 56330 }; // Rental item that enables AutoAttack
...
bool aa_check_target(map_session_data* sd, unsigned int id) {
	...
	// Verification du chemin et de la distance
	/*if (!check_distance_bl(sd, bl, range) && !unit_walktobl(sd, bl, 1, 1)) {
		return false;
	}

	// Verification du chemin et de la distance
	if (!check_distance_client_bl(sd, bl, range) && !unit_walktobl(sd, bl, 1, 1)){
		return false;
	}*/
...
int aa_status(map_session_data* sd) {
	...
	if (sd->max_weight > 0 && sd->weight >= (sd->max_weight * 90) / 100) {
...
bool aa_status_rest(map_session_data* sd, const status_data* status, t_tick last_tick) {
    ...
    bool is_overweight = 
#ifdef RENEWAL
        (sd->max_weight > 0 && sd->weight >= (sd->max_weight * 70) / 100);
#else
        (sd->max_weight > 0 && sd->weight >= (sd->max_weight * 50) / 100);
#endif
...
bool aa_status_attack(map_session_data* sd, struct mob_data* md_target, t_tick last_tick) {
	...
	std::shuffle(normal_skills.begin(), normal_skills.end(), gen);
...
int aa_get_random_coords(int16 m, int &x, int &y){
	...
	// R├®duction de l'├®chec en ├®largissant la zone
	for (int i = 0; i < 50; i++) {
...
// Fonction pour calculer le co├╗t heuristique (distance de Manhattan)
...
bool algorithm_path_finding(map_session_data* sd, int16_t m, int start_x, int start_y, int target_x, int target_y) {
    ...
    std::vector<float> cost_so_far(mapdata->xs * mapdata->ys, std::numeric_limits<float>::max());
    ...
    while (!open_list.empty() && max_attempt < 5000) {
        auto [_, current_x, current_y] = open_list.top();
        open_list.pop();
	...
	case 1:
		...
		if (battle_config.feature_autoattack_hateffect) {
			for (const auto& effectID : AA_HATEFFECTS) {
				...
				auto it = util::vector_get(sd->ud.hatEffects, effectID);

				if (it != sd->ud.hatEffects.end())
					continue;

				sd->ud.hatEffects.push_back(effectID);
	...
	case 2:
		...
		if (battle_config.feature_autoattack_hateffect) {
			for (const auto& effectID : AA_HATEFFECTS) {
				...
				util::vector_erase_if_exists(sd->ud.hatEffects, effectID);
