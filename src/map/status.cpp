...
int32 status_change_end( block_list* bl, enum sc_type type, int32 tid ){
	...
	switch(type) {
		...
		case SC__SHADOWFORM:
			...
			// An AFK AutoAttack character has no active socket. When its rental
			// status ends, release the native autotrade keep-alive as well.
			if (sd && sd->state.autotrade && sd->fd == 0) {
