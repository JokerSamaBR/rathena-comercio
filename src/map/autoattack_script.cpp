...
// Returns the highest normal job tree to expose in AutoAttack.
// Fourth-job characters intentionally use their third-job tree here.
static int32 autoattack_skill_tree_job(TBL_PC* sd) {
	uint64 mapid;

	if (sd->class_ & JOBL_THIRD)
		mapid = sd->class_ & MAPID_THIRDMASK;
	else if (sd->class_ & JOBL_2)
		mapid = sd->class_ & MAPID_SECONDMASK;
	else
		mapid = sd->class_ & MAPID_FIRSTMASK;

	return pc_mapid2jobid(mapid, sd->status.sex);
}

static bool autoattack_skill_allowed_for_class(TBL_PC* sd, uint16 skill_id) {
	if (skill_id == 0 || pc_checkskill(sd, skill_id) <= 0)
		return false;

	return skill_tree_get_max(skill_id, autoattack_skill_tree_job(sd)) > 0;
}

// Recupera somente os tipos de monstros que realmente existem no mapa atual.
// Monstros configurados em outros mapas não entram nesta lista; a configuração
// permanece salva e volta a aparecer quando o jogador retornar ao mapa correspondente.
	...
	while (true) {
		...
		mob_searched[md->mob_id] = true;
			...
			if (unique_mobs.count(md->mob_id) == 0) {
				bool active = find_aa_mob(sd, md->mob_id);
				std::ostringstream mob_temp;
				mob_temp << (active ? "^008000(ON)^000000 " : "^FF0000(OFF)^000000 ")
					<< "[" << mob->id << "] " << mob->name.c_str();
				unique_mobs[mob->id] = mob_temp.str();

				set_reg_num(st, sd, reference_uid(add_str(monster_menu_state), mob->id), monster_menu_state, active ? 1 : 0, nullptr);
			}
	}
	mapit_free(it);
...
std::map<int, std::string> get_unique_attack_skills(TBL_PC* sd, const char* skill_menu_state, script_state* st) {
	...
	if (!sd->aa.autoattackskills.empty()) {
		for (const auto& itAutoattackskills : sd->aa.autoattackskills) {
			if (!autoattack_skill_allowed_for_class(sd, itAutoattackskills.skill_id))
				continue;
	...
	for (int i = 0; i < MAX_SKILL; ++i) {
			if (sd->status.skill[i].id > 0 && sd->status.skill[i].lv > 0
				&& autoattack_skill_allowed_for_class(sd, sd->status.skill[i].id)
...
std::map<int, std::string> get_unique_buff_skills(TBL_PC* sd, const char* skill_menu_state, script_state* st) {
	...
	if (!sd->aa.autobuffskills.empty()) {
		for (const auto& itAutobuffskills : sd->aa.autobuffskills) {
			if (!autoattack_skill_allowed_for_class(sd, itAutobuffskills.skill_id))
				continue;
...
void format_skill_list_page(map_session_data* sd, std::ostringstream& menu, const std::map<int, std::string>& unique_skills, const char* skill_menu_ids, const char* skill_menu_total, const char* skill_menu_page_size, int page, script_state* st) {
	// Keep serialized select menus short enough for rAthena buildin_select.
			constexpr int page_size = 40;
	int total = static_cast<int>(unique_skills.size());
	...
	int page_index = 0;
	int current_index = 0;
	for (const auto& skill : unique_skills) {
		...
		if (page_index > 0)
			menu << ":";
		menu << skill.second;
		set_reg_num(st, sd, reference_uid(add_str(skill_menu_ids), ++page_index), skill_menu_ids, skill.first, nullptr);
	}

	set_reg_num(st, sd, reference_uid(add_str(skill_menu_total), 0), skill_menu_total, total, nullptr);
	set_reg_num(st, sd, reference_uid(add_str(skill_menu_page_size), 0), skill_menu_page_size, page_size, nullptr);
}
...
void handle_autoattack_monsterselection(std::ostringstream& os_buf, int index, int extra_index, script_state* st, map_session_data* sd) {
	...
	const char* mobs_menu_total = ".@mobs_menu_total";
	const char* mobs_menu_page_size = ".@mobs_menu_page_size";
	...
	if (index == 0) { // Monster menu
		...
		// Keep serialized select menus short enough for rAthena buildin_select.
			constexpr int page_size = 40;
		int page = extra_index < 0 ? 0 : extra_index;
		int total = static_cast<int>(unique_mobs.size());
		int start = page * page_size;
		...
		int page_index = 0;
		int current_index = 0;
		for (const auto& mob : unique_mobs) {
			if (current_index++ < start)
				continue;
			if (page_index >= page_size)
				break;
			...
			if (page_index > 0)
				menu << ":";
			menu << mob.second;
			set_reg_num(st, sd, reference_uid(add_str(mobs_menu_ids), ++page_index), mobs_menu_ids, mob.first, nullptr);
		}

		set_reg_num(st, sd, reference_uid(add_str(mobs_menu_total), 0), mobs_menu_total, total, nullptr);
		set_reg_num(st, sd, reference_uid(add_str(mobs_menu_page_size), 0), mobs_menu_page_size, page_size, nullptr);
...
void handle_autoattack_attack(std::ostringstream& os_buf, int index, int extra_index, script_state* st, map_session_data* sd) {
	...
	const char* skill_menu_total = ".@skill_menu_total";
	const char* skill_menu_page_size = ".@skill_menu_page_size";

	if (index == 0) { // Skill menu
		std::ostringstream menu;
		std::map<int, std::string> unique_skills = get_unique_attack_skills(sd, skill_menu_state, st);
		int page = extra_index < 0 ? 0 : extra_index;
		format_skill_list_page(sd, menu, unique_skills, skill_menu_ids, skill_menu_total, skill_menu_page_size, page, st);
