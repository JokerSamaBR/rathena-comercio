...
/** Enter AFK mode while AutoAttack remains active */
ACMD_FUNC(afk) {
	nullpo_retr(-1, sd);

	if (pc_isdead(sd)) {
		clif_displaymessage(fd, "You cannot enter AFK mode while dead.");
		return -1;
	}

	status_change* sc = status_get_sc(sd);
	if (!sd->state.autoattack || sc == nullptr || !sc->getSCE(SC_AUTOATTACK)) {
		clif_displaymessage(fd, "Start AutoAttack before entering AFK mode.");
		return -1;
	}

	if (sd->state.vending || sd->state.buyingstore || sd->state.autotrade) {
		clif_displaymessage(fd, "You cannot enter AFK mode while using a store.");
		return -1;
	}

	map_data* mapdata = map_getmapdata(sd->m);
	if (mapdata == nullptr) {
		clif_displaymessage(fd, "AFK mode is not available on this map.");
		return -1;
	}

	if (!battle_config.feature_autoattack_allow_town && mapdata->getMapFlag(MF_TOWN)) {
		clif_displaymessage(fd, "AutoAttack AFK mode is not allowed in town.");
		return -1;
	}

	if (!battle_config.feature_autoattack_allow_pvp && mapdata->getMapFlag(MF_PVP)) {
		clif_displaymessage(fd, "AutoAttack AFK mode is not allowed in PVP maps.");
		return -1;
	}

	if (!battle_config.feature_autoattack_allow_gvg && mapdata_flag_gvg2(mapdata)) {
		clif_displaymessage(fd, "AutoAttack AFK mode is not allowed in WoE maps.");
		return -1;
	}

	if (!battle_config.feature_autoattack_allow_bg && mapdata->getMapFlag(MF_BATTLEGROUND)) {
		clif_displaymessage(fd, "AutoAttack AFK mode is not allowed in Battleground maps.");
		return -1;
	}

	// Reuse rAthena's native autotrade disconnect path to keep the character
	// object on the map while the server-side SC_AUTOATTACK timer continues.
	sd->state.autotrade = 1;
	chrif_save(sd, CSAVE_AUTOTRADE);
