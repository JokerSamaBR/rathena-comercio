...
uint64 PopulationChatDatabase::parseBodyNode(const ryml::NodeRef& node)
{
	if (this->nodeExists(node, "ChatGlobal")) {
		...
		if (this->nodeExists(node, "ChatProximity")) {
			const ryml::NodeRef& root = node["ChatProximity"];
			PopulationChatProximityRule rule;
			population_append_string_list_top(root, "Keywords", rule.keywords);
			population_append_string_list_top(root, "Replies", rule.replies);
			if (rule.keywords.empty() || rule.replies.empty()) {
				this->invalidWarning(root, "ChatProximity requires non-empty Keywords and Replies lists.\n");
				return 1;
			}
			this->proximity_rules_.emplace_back(std::move(rule));
			return 1;
		}
...
const std::vector<PopulationChatProximityRule>& PopulationChatDatabase::proximity_rules() const
{
	return this->proximity_rules_;
}
