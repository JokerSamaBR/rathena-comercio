...
void mob_process_drop_list(std::shared_ptr<s_item_drop_list>& list, bool loot)
{
	...
	if (loot)
		...
		if (list->autofarm_owner_char_id != 0 && autofarm::consume_drop(list->autofarm_owner_char_id, ditem->item_data, ditem->item_data.amount))
			continue;
	...
	if (src) {
		...
		if (src->type == BL_PC) { // Player specific drop rate adjustments
			...
			if (sd->sc.getSCE(SC_ITEMBOOST))
				...
				const int32 season_drop_bonus = customrate_drop_rate(sd);
				if (season_drop_bonus > 1)
					drop_rate_bonus += season_drop_bonus;
...
int32 mob_dead(mob_data *md, block_list *src, int32 type)
{
	...
	const uint32 autofarm_owner = autofarm::owner_char_id(src);
	if (autofarm_owner != 0)
		autofarm::record_kill(autofarm_owner);
	...
	lootlist->autofarm_owner_char_id = autofarm_owner;
	...
	{ // Item Drop
		...
		dlist->autofarm_owner_char_id = autofarm_owner;
