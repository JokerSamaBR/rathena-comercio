...
static std::unordered_map<int32, t_tick> g_pop_chat_next_tick; ///< Per-shell ambient/name-reply eligibility tick.
static std::unordered_map<int32, t_tick> g_pop_chat_proximity_next_tick; ///< Per-shell proximity FAQ cooldown.
...
static std::string population_engine_chat_lower_ascii(const char* msg)
{
	if (!msg)
		return {};
	std::string lower(msg);
...
static bool population_engine_chat_keyword_match(const std::string& message_lower, const std::string& keyword)
{
	if (message_lower.empty() || keyword.empty())
		return false;
	std::string keyword_lower = keyword;
	std::transform(keyword_lower.begin(), keyword_lower.end(), keyword_lower.begin(), [](unsigned char c) {
		return static_cast<char>(::tolower(c));
	});
	return message_lower.find(keyword_lower) != std::string::npos;
}
...
/// Deliver one configured ChatProximity reply to a real player standing near bot_sd.
static bool population_engine_deliver_proximity_reply_locked(map_session_data* bot_sd, const char* message)
{
	extern struct Battle_Config battle_config;
	if (!bot_sd || !message || !message[0])
		return false;
if (!battle_config.population_engine_chat_enable ||
			!battle_config.population_engine_chat_proximity_enable)
		return false;
	if (bot_sd->prev == nullptr || map_id2bl(bot_sd->id) != bot_sd || !bot_sd->status.name[0])
		return false;

	const std::string message_lower = population_engine_chat_lower_ascii(message);
	const PopulationChatProximityRule* matched = nullptr;
	for (const PopulationChatProximityRule& rule : population_chat_db().proximity_rules()) {
		for (const std::string& keyword : rule.keywords) {
			if (population_engine_chat_keyword_match(message_lower, keyword)) {
				matched = &rule;
				break;
			}
		}
	...
	if (matched == nullptr || matched->replies.empty())
		return false;

	const t_tick now = gettick();
	const auto it = g_pop_chat_proximity_next_tick.find(bot_sd->id);
	if (it != g_pop_chat_proximity_next_tick.end() && now < it->second)
		return false;

	const std::string& pick = matched->replies[rnd() % matched->replies.size()];
	char buf[CHAT_SIZE_MAX];
	population_engine_format_chat_line(bot_sd, pick.c_str(), buf, sizeof(buf));
	if (population_engine_chat_line_blocked(buf))
		return false;
	...
	const int32 base_cd = battle_config.population_engine_chat_proximity_cooldown_ms;
	const int32 jit = battle_config.population_engine_chat_cooldown_jitter_ms;
	const t_tick add = static_cast<t_tick>(base_cd +
		(jit > 0 ? static_cast<int32>(rnd() % (static_cast<uint32_t>(jit) + 1u)) : 0));
	g_pop_chat_proximity_next_tick[bot_sd->id] = now + add;
	return true;
}
...
void population_engine_shell_release(map_session_data* sd)
{
	...
	g_pop_chat_next_tick.erase(sd->id);
	g_pop_chat_proximity_next_tick.erase(sd->id);
	population_engine_path_erase_for_pc(sd->id);
	...
	PopulationDbSource db_source)
{
	...
	sd->group_id = 1; // Id: 1 is the normal Player group in conf/groups.yml.
	pc_group_pc_load(sd);
	if (!sd->group) {
		ShowError("Population engine: no valid group (group_id=%d) for population shell %u; using minimal defaults.\n", sd->group_id, index);
...
void population_engine_stop() {
	...
	g_pop_chat_proximity_next_tick.clear();
	population_engine_path_clear_all();
    g_chat_cursor = 0;
...
void population_engine_on_global_chat_mention(map_session_data* from_sd, const char* message)
{
	...
	if (population_engine_is_population_pc(from_sd->id))
		return;
	if (!from_sd->state.active || from_sd->prev == nullptr)
		return;
...
if (!battle_config.population_engine_chat_enable)
	...
	// Proximity FAQ: only the nearest active shell on the same map and within the
	// configured cell range is eligible. The delivery helper still requires a
	// configured keyword match and enforces the shell-specific cooldown.
	if (battle_config.population_engine_chat_proximity_enable && battle_config.population_engine_chat_proximity_range > 0) {
		map_session_data* nearest = nullptr;
		int nearest_distance = INT_MAX;
		for (map_session_data* bot : g_population_engine_pcs) {
			if (!bot || !bot->state.active || bot->prev == nullptr)
				continue;
			if (map_id2bl(bot->id) != bot || bot->m != from_sd->m || bot->id == from_sd->id)
				continue;
const int proximity_distance = distance_xy(from_sd->x, from_sd->y, bot->x, bot->y);
				if (proximity_distance > battle_config.population_engine_chat_proximity_range || proximity_distance >= nearest_distance)
					continue;
				nearest = bot;
				nearest_distance = proximity_distance;
		}
		if (nearest != nullptr && population_engine_deliver_proximity_reply_locked(nearest, message))
			return;
	}

	// Name-mention replies remain independently controlled by their own setting.
	if (!battle_config.population_engine_chat_reply_enable)
		return;

	// Preserve existing behavior: a shell can answer when its name is mentioned
	// in public map chat, even if the player is not standing nearby.
	for (map_session_data* bot : g_population_engine_pcs) {
		if (!bot || !bot->state.active || bot->prev == nullptr)
			continue;
